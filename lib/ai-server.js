...
function demoScan(user) {
  ...
  return {
    summary: `Typical live openings for ${o.name}. Confirm each against the source links below — Atlas infers these from how programs of this type usually run, and does not yet crawl the platform directly.`,
    openings: [
...
function demoLeadBrief(user) {
  ...
  return {
    overview: `${l.name} is ${l.role} at ${l.company}. ${l.notes ? l.notes + " " : ""}On the evidence available, they sit close enough to the buying decision to be worth a direct, specific approach rather than a nurture sequence.`,
    fit: `${l.company} has the surface area to use what ${name} builds without a long integration.`,
    priority: 45 + (seed % 50),
    angle: `Lead with a concrete result ${l.company} would get in the first month — not a product tour.`,
...
function demoFollowups(user) {
  ...
  return {
    ...
    cadence_note: "Four touches over ~16 days, each shorter than the last, with an explicit exit. Stop the sequence the moment they reply.",
  };
}
...
export async function runAI({ system = "", user = "" }) {
  ...
  if (truthy(process.env.ATLAS_DEMO_MODE) || !apiKey) {
