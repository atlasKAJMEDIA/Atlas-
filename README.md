...
- **Demo mode (free, $0).** Leave `VIRTUALS_API_KEY` unset — or set `ATLAS_DEMO_MODE=1` — and the app generates realistic, project-specific content locally for every AI feature. No API calls, no billing, nothing can fail from a bad key or blocked network. **This is the recommended setting for demonstrations.**
...
To run a full demo for free: deploy with **no** `VIRTUALS_API_KEY`, or add `ATLAS_DEMO_MODE=1`. That's the only setting change needed.
...
4. **For a free demo, add nothing** — just deploy. (Or set `ATLAS_DEMO_MODE=1` in **Project → Settings → Environment Variables** to be explicit.) Every feature works at $0.
...
6. Click **Deploy**. You'll get a live URL like `atlas-xyz.vercel.app`. Every future push auto-redeploys.
